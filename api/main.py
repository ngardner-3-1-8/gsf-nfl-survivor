...
def optimize(request: OptimizeRequest):
    try:
        ...
        if target_year == current_year:
            ...
            # Historical: the cumulative season file has ALL weeks' games.
            # (The per-week sim files each hold only a single week.)
            import glob as _glob
            files = _glob.glob(os.path.join(
                DATA_DIR,
                f"nfl-power-ratings/final_data/{target_year}_final_data/"
                f"Season_{target_year}_Through_Week_*_Final_Data.csv"))
            if not files:
                raise FileNotFoundError(f"No season data for {target_year}")
            def wk(p):
                try:
